      ...
      <Layout>
        ...
        <Content :style="{padding: '0 24px 24px 24px', minHeight: '280px', background: '#fff'}">
          ...
          <div v-show="currName=='1-0'">
            ...
            <h3 class="article">说明</h3>如果你觉得上方YBoot提供的动画不明显，你还可以使用iView官方自带的顶部
            <a
          ...
          <div v-show="currName=='3-1'">
            <Alert type="info" show-icon>
              基于
              <a href="http://www.wangeditor.com" target="_blank">wangEditor</a> 封装，已配置好图片上传(上传至YBoot文件服务或Base64)、表情包。扩展编辑HTML代码、全屏预览、清空、XSS攻击过滤等。
            </Alert>
            ...
            <h3 class="article">样式冲突</h3>在
            <code>Form-item</code> 中使用时，建议加上 <code>class="form-wangEditor"</code>。参考代码生成器中的用法。
            <h3 class="article">props</h3>
          ...
          <div v-show="currName=='3-2'">
            <Alert type="info" show-icon>
              基于
              <a href="https://github.com/quilljs/quill" target="_blank">quill</a> 封装。已配置好图片上传(上传至YBoot文件服务或Base64)。扩展编辑HTML代码、全屏预览、清空、XSS攻击过滤等。
            </Alert>
            <quill id="quill" v-model="quillData"></quill>
            <h3 class="article">基础用法</h3>使用
            <code>v-model</code> 实现数据的双向绑定。单页面同时使用两个及以上该组件时，需设定不同的id值加以区分。
            <h3 class="article">样式冲突</h3>在
            <code>Form-item</code> 中使用时，建议加上 <code>class="form-quill"</code>。参考代码生成器中的用法。
            <h3 class="article">props</h3>
          ...
          <div v-show="currName=='2-1'">
            ...
            <div style="font-size:12px;margin-bottom:10px">示例（仅为演示传入ActBusinessId为测试数据123456，所以会报错）：</div>
          ...
          <div v-show="currName=='2-2'">
            ...
            >示例（仅为演示传入流程key为请假申请leave，传入ActBusinessId为测试数据123456，所以会报错）：</div>
          ...
          <div v-show="currName=='2-3'">
            ...
            <div
              style="font-size:12px;margin-bottom:10px"
            >示例（仅为演示传入ActBusinessId和流程实例的procInstId均为测试数据123456，所以会报错）：</div>
...
import iconChoose from "@/views/my-components/yboot/icon-choose";
import countDownButton from "@/views/my-components/yboot/count-down-button";
import departmentChoose from "@/views/my-components/yboot/department-choose";
import departmentTreeChoose from "@/views/my-components/yboot/department-tree-choose";
import userChoose from "@/views/my-components/yboot/user-choose";
import uploadPicInput from "@/views/my-components/yboot/upload-pic-input";
import uploadPicThumb from "@/views/my-components/yboot/upload-pic-thumb";
import checkPassword from "@/views/my-components/yboot/check-password";
import setPassword from "@/views/my-components/yboot/set-password";
import editor from "@/views/my-components/yboot/editor";
import quill from "@/views/my-components/yboot/quill";
import processChoose from "@/views/my-components/yboot/process-choose";
import processStart from "@/views/my-components/yboot/process-start";
import processCancel from "@/views/my-components/yboot/process-cancel";
export default {
  name: "yboot-components",
  components: {
    TreeTable,
    iconChoose,
    countDownButton,
    ...
    return {
      ...
      data30: [
        ...
        },
        {
          name: "base64",
          desc:
            "是否使用base64保存图片，默认false上传至YBoot配置的文件存储服务中，不推荐使用base64存储",
          type: "Boolean",
          value: "false"
        },
        {
