...
util.initRouter = function (vm) {
    ...
    if (!vm.$store.state.app.added) {
        ...
        // 加载菜单
        axios.get(getMenuList, { headers: { 'accessToken': accessToken,'Authorization': 'bearer '+accessToken } }).then(res => {
            // 获取
            let menuData = res.result;
            if (!menuData) {
                return;
            }
            // constRoutes 存储了该用户权限的所有路由
...
util.initRouterNode = function (routers, data) {
    ...
    for (var item of data) {
        ...
        meta.title = menu.title ? menu.title + " - YBoot前后端分离开发平台" : null;
